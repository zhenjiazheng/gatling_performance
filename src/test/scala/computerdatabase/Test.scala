...
//  val feeder = Array(Map("mobile" -> ("170" + Random.nextInt(end - start + 1 ).toString()))).random
//  println(feeder)
...
//      .doIf(session => !session.contains("token"))
        .tryMax(1) {
          ...
          .check(status.is(200), jsonPath("$.data.token").saveAs("token"), jsonPath("$.statusCode").saveAs("statusCode"), jsonPath("$.data").saveAs("data")))
           }
        .pause(2)
//      .exec { session =>
//        println(session)
...
//    notice.inject(constantUsersPerSec(1) during(10)),
    cart.inject(constantUsersPerSec(30) during(2))
  ).protocols(httpConf)
}
